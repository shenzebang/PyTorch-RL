...
def trpo_step(policy_net, value_net, states, actions, returns, advantages, max_kl, damping, l2_reg, newton_weight, use_fim=False):
    ...
    def Fvp_direct(v):
        ...
        loss = get_loss()
        grads = torch.autograd.grad(loss, policy_net.parameters(), create_graph=True)
        loss_grad = torch.cat([grad.view(-1) for grad in grads])
        loss_v = torch.dot(loss_grad, v)
        grads = torch.autograd.grad(loss_v, policy_net.parameters())
        flat_grad_grad_loss = torch.cat([grad.contiguous().view(-1) for grad in grads]).detach()

        return flat_grad_grad_kl + flat_grad_grad_loss * newton_weight + v * damping
    ...
    print(torch.norm(Fvp(stepdir) + loss_grad)/torch.norm(loss_grad))
